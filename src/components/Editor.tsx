...
export function Editor({ file, status, error, saving, onChange, onSave }: { file: OpenedFile | null; status: EditorStatus; error?: string; saving: boolean; onChange: (content: string) => void; onSave: () => void }) {
  ...
  return <section className="editor-panel" aria-labelledby="editor-heading">
    ...
    {status !== "loading" && status !== "error" && file && <div className="monaco-container"><MonacoEditor path={file.path} language={editorLanguage(file.name)} value={file.content} theme="vs-dark" onChange={(value) => onChange(value ?? "")} options={{ automaticLayout: true, minimap: { enabled: false }, wordWrap: "on" }} /></div>}
