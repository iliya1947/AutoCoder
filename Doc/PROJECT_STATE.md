...
31 августа 2026 (добавлено явное управление остановкой активной orchestration task).
...
- Остановка инвалидирует ожидаемый model response, убирает review/recovery UI и не позволяет позднему
  ответу дописать Chat или перезаписать terminal state. Уже запущенный внешний процесс не завершается
  скрыто: существующие гарантии Terminal process lifecycle и отдельная история результата сохранены.
