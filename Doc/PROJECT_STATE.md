...
29 августа 2026 (packaged Windows-проверка Terminal project-switch guard пройдена; ошибка открытия проекта теперь показывает backend-диагностику).
...
- следующим отдельным шагом проверить в packaged Windows, что новая UI-диагностика показывает точную причину отказа при блокировке смены проекта, не скрывая доступный Cancel;
...
Основной `Doc/WINDOWS_CHAT_CHECKLIST.md`, Backup Manager, Terminal Tool, AI → Terminal review, Terminal → Chat review, три негативных diagnostic scenarios Ollama и изоляция Chat при смене проекта успешно подтверждены пользователем на packaged Windows-сборке. Terminal project-switch guard также подтверждён в packaged Windows: блокировка, доступный Cancel, успешный повторный выбор и отмена folder dialog работают корректно. Следующим техническим шагом выполнено отображение точной backend-диагностики ошибки открытия проекта в Project Explorer; ближайшая отдельная проверка — подтвердить этот текст в packaged Windows.
