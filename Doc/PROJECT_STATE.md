...
28 августа 2026 (добавлен просмотр и безопасное восстановление backup-копий).

## Текущий этап

**Этап 4 — начата реализация минимально полезного MVP: подключён первый вертикальный AI Chat.**

Файловый сценарий, backup и основной packaged AI Chat фактически подтверждены на Windows. Чат передаёт историю, текущий текст открытого файла и read-only структуру проекта через Tauri в минимальный Python-backend и получает ответ локального Ollama; безопасные предложения изменения, создания и удаления файла подключены, доступен просмотр и восстановление backup-копий, а постоянное хранение истории пока не реализовано.
...
- Добавлены Rust-регрессии фильтрации чужих backup, stale-состояния, резервирования заменяемой версии и восстановления удалённого файла. Frontend-тесты и production/offline-сборка проходят в Linux. Следующий шаг — ручная проверка менеджера backup в packaged Windows-приложении; после неё минимальный Terminal Tool выполнять отдельной задачей.
- В `Doc/WINDOWS_CHAT_CHECKLIST.md` добавлен воспроизводимый Windows-сценарий для существующего и удалённого файла, stale-состояния и отмены при несохранённых изменениях. Сам runtime-результат этой ручной проверки пока не получен и не объявляется подтверждённым.
...
- Добавлены frontend-, backend- и Rust-регрессии для clean/dirty/stale proposal, внешнего изменения диска, точного backup и запрета traversal. Следующий отдельный шаг — предоставить минимальный просмотр и восстановление созданных backup-копий; Terminal Tool до этого не расширять.
...
- ручная Windows-проверка просмотра и восстановления backup-копий;
- Terminal Tool;
- SQLite;
...
1. Просмотр и восстановление backup-копий автоматизированно проверены в Linux, но ещё не подтверждены в packaged Windows-приложении.
2. Terminal Tool ещё не реализован и должен начинаться только после указанной Windows-проверки backup.
...
- вручную проверить просмотр и восстановление существующего и удалённого файла в packaged Windows-приложении;
- после этой проверки реализовать минимальный Terminal Tool отдельной следующей задачей;
