...
28 августа 2026 (добавлен воспроизводимый сценарий Windows-проверки AI Chat).
...
- Сам Windows-сценарий в Linux-контейнере не выполнен и по-прежнему требует целевой Windows 10/11; до его успешного прохождения File Tool не расширяется.
...
3. Постоянный identifier, сведения о desktop crate и минимальный CSP настроены, но production CSP ещё требуется проверить в packaged Windows-сборке вместе с Monaco и Tauri IPC. Лицензия самого закрытого коммерческого проекта намеренно не объявлена.
...
Следующий шаг — выполнить `Doc/WINDOWS_CHAT_CHECKLIST.md` с реальным Ollama и packaged AutoCoder на Windows и зафиксировать результат здесь. Не расширять File Tool на создание или удаление файлов до проверки безопасного цикла предложения и подтверждения.
