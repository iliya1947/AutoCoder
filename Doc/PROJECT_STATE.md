...
29 августа 2026 (packaged Windows-проверка изоляции Chat пройдена; смена проекта защищена от активной Terminal-команды).
...
- Backend теперь не меняет текущий проект, если после выбора новой папки всё ещё выполняется Terminal-команда. Пользователь должен дождаться завершения либо явно отменить её и повторить выбор; отмена системного выбора папки, как и прежде, не меняет текущую сессию.
- Добавлен Rust regression состояния active command. Автоматические проверки выполнены в Linux; точное сообщение блокировки и повторный выбор после Cancel остаются ближайшим packaged Windows regression.
...
- вручную подтвердить на packaged Windows, что активная Terminal-команда блокирует фактическую смену проекта, остаётся доступна для Cancel, а после её отмены повторный выбор успешно открывает новый проект;
...
Основной `Doc/WINDOWS_CHAT_CHECKLIST.md`, Backup Manager, Terminal Tool, AI → Terminal review, Terminal → Chat review, три негативных diagnostic scenarios Ollama и изоляция Chat при смене проекта успешно подтверждены пользователем на packaged Windows-сборке. По результату ревизии следующий lifecycle-риск устранён: активная Terminal-команда блокирует смену проекта до завершения или явной отмены. Ближайшая проверка — packaged Windows regression этой блокировки и успешного повторного выбора.
