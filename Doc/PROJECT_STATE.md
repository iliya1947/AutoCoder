...
30 августа 2026 (реализована project-scoped SQLite persistence истории Chat и Terminal; требуется packaged Windows acceptance).
...
- Rust regression проверяет запись, повторное открытие базы, изоляцию проектов и независимую очистку.
  Frontend tests (40), production/offline build и Python tests (33) проходят.
...
- Требуется критическая packaged Windows-проверка: SQLite инициализируется при старте desktop runtime,
  поэтому нужно подтвердить запуск packaged app, восстановление после перезапуска, изоляцию двух
  проектов и независимую очистку. Без подтверждения нельзя считать новый фундамент надёжным.
