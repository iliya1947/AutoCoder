...
29 августа 2026 (история Terminal подтверждена на packaged Windows; реализована явная отмена текущей команды).
...
- Общий lifecycle остаётся внешней границей владения и по-прежнему завершает все процессы AutoCoder при shutdown. Linux-проверки frontend/build проходят; точное завершение дерева команды без воздействия на Ollama/Python ожидает packaged Windows-проверки.
...
- проверить в packaged Windows явную отмену долгой terminal-команды с descendant и убедиться, что принадлежащие AutoCoder Ollama/Python продолжают работать;
