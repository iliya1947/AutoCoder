...
29 августа 2026 (Terminal → Chat подтверждён; диагностика Ollama проверена и HTTP 503 исправлена автоматизированно).
...
- Добавлены regression-тесты Python, подтверждающие HTTP 503 без поиска executable, и Rust, подтверждающий сохранение status 503. Существующие тесты покрывают отсутствующий Ollama, отсутствующую модель и запрет chat request без модели. Реальный packaged Windows-прогон трёх негативных сценариев ещё не выполнен и не объявляется подтверждённым.
...
- подтвердить в packaged Windows автоматизированно проверенную диагностику отсутствующего `ollama.exe`, отсутствующей модели и HTTP 503 readiness по готовому `Doc/WINDOWS_CHAT_CHECKLIST.md`;
- зафиксировать точные сообщения UI и диагностические строки при сбое, не понижая уже подтверждённые основные сценарии;
...
Основной `Doc/WINDOWS_CHAT_CHECKLIST.md`, Backup Manager, Terminal Tool, AI → Terminal review и Terminal → Chat review успешно подтверждены пользователем на packaged Windows-сборке. Следующий ограниченный шаг — отдельно пройти негативные diagnostic scenarios Ollama (отсутствующий executable, отсутствующая модель и HTTP 503 readiness); до фактического прогона не выдавать их за подтверждённые.
