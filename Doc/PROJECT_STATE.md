...
29 августа 2026 (AI → Terminal review flow подтверждён на packaged Windows; добавлена история команд текущего проекта).
...
- История очищается при закрытии проекта, не переносит команды между проектами и не сохраняется на диск. Следующий отдельный шаг — спроектировать явную отмену одной длительной команды без завершения общего Job Object и принадлежащих приложению Ollama/Python процессов.
...
2. Terminal Tool хранит непостоянную историю команд текущего проекта, но пока не умеет отменять отдельный длительный процесс; AI не имеет автоматического доступа к запуску и может только заполнить редактируемое поле после подтверждения пользователя.
...
- спроектировать и реализовать явную отмену одной длительной terminal-команды так, чтобы не завершать общий Windows Job Object вместе с Ollama/Python;
- не передавать результат команды обратно AI и не добавлять автоматический запуск;
