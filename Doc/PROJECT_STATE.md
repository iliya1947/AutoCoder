...
30 августа 2026 (реализована project-scoped persistence истории Chat и Terminal; повторная packaged-проверка Refresh ожидается).

## Текущий этап

**Этап 4 — завершается универсальное ядро; COMSOL перенесён к финалу проекта.**

### Универсальная persistence истории (30 августа 2026)

- Добавлена локальная SQLite-база в app data AutoCoder. История изолирована каноническим путём
  проекта и не размещается внутри пользовательского проекта.
- Успешные пары сообщений Chat сохраняются транзакционно и восстанавливаются после перезапуска;
  неуспешные/отброшенные ответы не создают ложную завершённую пару.
- Команды Terminal сохраняются при фактическом запуске, повтор соседней одинаковой команды не
  дублируется; стрелки истории работают с восстановленными командами.
- Chat и Terminal имеют независимую очистку истории текущего проекта. Переключение проекта
  загружает только соответствующую историю, сохраняя существующую session/race изоляцию.
- Добавлены Rust-тесты схемы, project isolation, deduplication и независимой очистки; frontend
  regressions согласованы с новыми IPC загрузки. Frontend 40/40, Rust 44/44, backend 33/33,
  production build, offline-runtime и format/diff checks проходят в Linux-контейнере после
  установки системных Tauri build-зависимостей.
- Фактическая общая готовность после пакета оценивается в **66%** на той же продуктовой шкале:
  пакет закрывает существенный session/history пробел, но не означает автоматический барьер 70%.
  До 70% остаются восстановление последнего workspace/open-file state, более цельный список
  прошлых terminal results/AI changes и закрытие pending 60% Refresh recheck.
- Изменение SQLite/Tauri lifecycle требует packaged Windows проверки на 70% barrier: перезапуск,
  project isolation, очистка и запись истории. Немедленная критическая проверка не требуется:
  ошибки persistence не блокируют открытие/редактирование/сохранение проекта, а автоматические
  проверки закрывают общую логику.

Файловый сценарий, backup, Backup Manager, основной AI Chat, минимальный Terminal Tool, AI → Terminal review flow и все три негативных сценария Ollama фактически подтверждены в packaged Windows. Чат передаёт историю, текущий текст открытого файла и read-only структуру проекта через Tauri в минимальный Python-backend и получает ответ локального Ollama; безопасные предложения файловых операций и команды терминала подключены. Команда ИИ только переносится в редактируемое поле Terminal после явного действия пользователя и требует отдельного нажатия «Выполнить». Истории Chat и команд Terminal теперь сохраняются по проекту в SQLite; автоматический запуск команд и автоматическая отправка результата модели остаются запрещены.
...
- **60% packaged Windows acceptance остаётся FAIL / pending recheck**: автоматические проверки не
  заменяют требуемую пользователем повторную проверку исправленной packaged Windows-сборки.
...
### Поздний этап — первая специализация COMSOL
Только после практически полного универсального AutoCoder:
...
До практически полного завершения общего ядра не создавать COMSOL corpus/retrieval/UI/backend,
knowledge engine или подготовительную инфраструктуру. PR #79 отражает преждевременный порядок и
не считается следующим этапом.
