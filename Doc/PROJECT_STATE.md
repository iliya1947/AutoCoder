...
29 августа 2026 (обычный Save защищён от перезаписи внешних изменений файла).
...
- Изменение относится к защите от потери данных, поэтому по правилам проекта требуется немедленная
  packaged Windows-проверка: открыть файл, изменить его в AutoCoder, затем изменить на диске внешним
  редактором и нажать Save. AutoCoder должен показать `The file changed on disk after it was opened.`,
  не перезаписать внешний текст и оставить редактор dirty. До этого runtime-статус не считать PASS.
...
- следующий обязательный шаг — немедленно выполнить описанную выше packaged Windows-проверку
  stale Save, поскольку изменение защищает пользовательский файл от перезаписи;
