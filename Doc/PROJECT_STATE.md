...
29 августа 2026 (50% acceptance barrier подтверждён в packaged Windows со статусом PASS).

## Текущий этап

**Этап 4 — минимально полезный MVP собран; после PASS 50% barrier закрывается недостающий Project Manager flow к барьеру 60%.**
...
- добавлено явное обновление дерева после внешних изменений; следующий шаг — оценить минимальный
  состав 60% barrier и только затем выбрать следующий недостающий пользовательский flow либо
  переход к Этапу 5, не повторяя завершённую общую frontend async/race ревизию;
...
Следующим недостающим MVP-шагом реализовано явное обновление дерева после внешних изменений без
смены session-only сессии. Диагностические Windows atomic replacement branches 1175–1177 остаются
engineering backlog, а не обычным пользовательским acceptance blocker.
