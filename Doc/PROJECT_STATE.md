...
25 августа 2026 (обновлено после добавления Windows-диагностики Project Explorer).
...
### Диагностика Project Explorer на Windows (25 августа 2026)
- После удаления фильтрации расширений пользователь подтвердил на Windows: 7/7 Rust-тестов проходят и `npm run tauri dev` запускается, однако Project Explorer показывает каталоги и не показывает обычные файлы, включая `АвтоКодер_тестовый файл.txt`. Поэтому whitelist расширений больше не рассматривается как причина этой сохраняющейся проблемы.
- В debug-сборку добавлена диагностика полного runtime-пути без чтения и вывода содержимого файлов: исходный путь из Tauri folder picker, канонический физический путь, результат обхода `read_dir`, имя и тип каждой записи, ошибки получения записи/типа, Windows file attributes и reparse-point flag, точная причина каждого исключения, JSON итогового `ProjectTree`, а также полученное и преобразованное дерево в консоли frontend.
- Отличие выбранной Windows Shell-папки/библиотеки от физического каталога теперь можно проверить сравнением пути picker с каноническим путём и фактическими записями корневого `read_dir`. Причина отсутствия файлов пока не установлена: для этого нужен диагностический вывод с целевой Windows-машины.
