...
- Добавлены regression-тесты built-in ASCII stdout/exit 0, built-in Unicode stdout/stderr, external executable с детерминированными ASCII и UTF-8 stdout/stderr, ненулевого exit code, реального cwd, Unicode project directory, сохранения настоящего UNC-представления и shutdown долгого shell с descendant при сохранении внешнего процесса. Следующий шаг — повторить эти regression-сценарии в packaged Windows-сборке; до этого blocker-ы исправлены в коде, но новую сборку нельзя считать фактически подтверждённой на Windows.
