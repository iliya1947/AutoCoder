...
- Удаление выполняется только после отдельного нажатия пользователя. Tauri заново разрешает и проверяет путь внутри проекта, сохраняет содержимое и metadata в обязательную backup-копию, повторно проверяет путь перед удалением, затем обновляет дерево и закрывает удалённый файл в редакторе.
- Добавлены frontend-, backend- и Rust-регрессии для актуальности proposal, ограничения текущим файлом, сохранения backup и запрета traversal. Следующий отдельный шаг — предоставить минимальный просмотр и восстановление созданных backup-копий; Terminal Tool до этого не расширять.
...
- добавить минимальный просмотр и безопасное восстановление созданных backup-копий;
- Terminal Tool выполнять отдельной следующей задачей;
...
Основной `Doc/WINDOWS_CHAT_CHECKLIST.md` успешно выполнен пользователем на packaged Windows-сборке. Следующий шаг — минимальный просмотр и безопасное восстановление созданных backup-копий; отдельно непроверенные негативные diagnostic scenarios Ollama не выдавать за пройденные.
