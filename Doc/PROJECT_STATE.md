...
1. Нет автоматических Rust-тестов для ограничения путей, чтения, сохранения и создания backup. Эти операции являются наиболее критичной частью уже реализованного сценария.
2. Фильтрация поддерживаемых текстовых файлов выполняется во frontend после получения дерева. Backend всё ещё обходит и передаёт имена всех обычных файлов, а списки исключённых каталогов frontend/backend различаются.
3. Запись файла не атомарна, управление и восстановление backup-копий отсутствуют. Текущий backup защищает исходную версию перед записью, но сбой во время `fs::write` может оставить целевой файл частично записанным.
4. Desktop-конфигурация сохраняет шаблонный идентификатор `com.tauri.dev`, пустые сведения о лицензии/репозитории и отключённый CSP. Эти параметры нужно исправить до распространения сборок, но они не блокируют локальную проверку MVP.
5. Чат остаётся UI-заглушкой; Python-backend, контракт провайдера моделей и хранение истории ещё не спроектированы в коде.
...
- добавить unit-тестируемое Rust-ядро файловых операций и тесты защиты от выхода за корень проекта, чтения, сохранения и создания backup;
- синхронизировать backend-фильтрацию дерева с frontend и не передавать в UI заведомо неподдерживаемые файлы;
- проверить полный сценарий «открыть проект → открыть файл → изменить → сохранить → проверить backup» в desktop-сборке на Windows 10/11 и зафиксировать результат;
